...
public class ServiceReferencesMetaDataMerger {
    ...
    public void augment(ServiceReferencesMetaData dest, ServiceReferencesMetaData augment, ServiceReferencesMetaData main, boolean resolveConflicts) {
        for (ServiceReferenceMetaData serviceRef : augment) {
            if (dest.containsKey(serviceRef.getKey())) {
                if (!resolveConflicts && (main == null || !main.containsKey(serviceRef.getKey()))) {
                    ...
                    dest.get(serviceRef.getKey()).augment(serviceRef, (main != null) ? main.get(serviceRef.getKey()) : null,
                            resolveConflicts);
                }
            } else {
                dest.add(serviceRef);
