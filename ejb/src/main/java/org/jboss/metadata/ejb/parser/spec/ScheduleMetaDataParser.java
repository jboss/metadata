...
public class ScheduleMetaDataParser extends  AbstractMetaDataParser<ScheduleMetaData>
{
    ...
    */
   @Override
   public ScheduleMetaData parse(XMLStreamReader reader) throws XMLStreamException
   {
    ...
    */
   @Override
   protected void processElement(ScheduleMetaData schedule, XMLStreamReader reader) throws XMLStreamException
   {
      final EjbJarElement ejbJarElement = EjbJarElement.forName(reader.getLocalName());
      switch (ejbJarElement)
      {
         case SECOND:
            String second = getElementText(reader);
            schedule.setSecond(second);
            return;

         case MINUTE:
            String minute = getElementText(reader);
            schedule.setMinute(minute);
            return;

         case HOUR:
            String hour = getElementText(reader);
            schedule.setHour(hour);
            return;

         case DAY_OF_MONTH:
            String dayOfMonth = getElementText(reader);
            schedule.setDayOfMonth(dayOfMonth);
            return;

         case MONTH:
            String month = getElementText(reader);
            schedule.setMonth(month);
            return;

         case YEAR:
            String year = getElementText(reader);
            schedule.setYear(year);
            return;

         case DAY_OF_WEEK:
            String dayOfWeek = getElementText(reader);
