...
public class JBoss5xEverythingUnitTestCase extends AbstractEJBEverythingTest {
    ...
    public void assertEverything(JBossMetaData jbossMetaData, Mode mode) {
        ...
        if (mode == Mode.JBOSS_DTD) {
            if (hasStandardJBoss == false) { assertInvokerProxyBindings(jbossMetaData.getInvokerProxyBindings(), mode); }
    ...
    public void assertWebservices(WebservicesMetaData webservices, Mode mode) {
        ...
        if (mode != Mode.JBOSS_DTD) { assertId("webservices", webservices); }
        ...
        for (WebserviceDescriptionMetaData description : webserviceDescriptionsMetaData) {
            if (mode != Mode.JBOSS_DTD) { assertId("webserviceDescription" + count, description); }
    ...
    */
    private void assertFullSessionBean(String prefix, JBossMetaData jbossMetaData, boolean first, Mode mode, int num) {
        ...
        if (mode != Mode.JBOSS_DTD) {
            ...
            if (first) { assertTrue(session.isConcurrent()); } else { assertFalse(session.isConcurrent()); }
        }
        ...
        if (mode == Mode.JBOSS) { clusterConfig = session.getClusterConfig(); } else { clusterConfig = session.determineClusterConfig(); }
...
*/
    ...
    */
    ...
    private ContainerConfigurationMetaData assertContainerConfiguration(String prefix, JBossMetaData jBossMetaData) {
        ...
        } else if (policy != null) { assertEquals(prefix + "LockingPolicy", policy); }
    ...
    private void assertNullContainerConfiguration(String prefix, JBossMetaData jBossMetaData) {
        ...
        if (this.hasStandardJBoss) { assertEquals("org.jboss.web.WebClassLoader", configuration.getWebClassLoader()); } else {
            assertEquals(null, configuration.getWebClassLoader());
        }
        if (this.hasStandardJBoss) { assertEquals("org.jboss.ejb.plugins.lock.QueuedPessimisticEJBLock", configuration.getLockingPolicy()); } else {
        ...
        if (this.hasStandardJBoss) { assertEquals(30000, configuration.getOptiondRefreshRateMillis()); } else {
            assertEquals(0, configuration.getOptiondRefreshRateMillis());
        }
        assertNull(configuration.getClusterConfig());
        if (this.hasStandardJBoss) { assertTrue(configuration.getDepends().isEmpty()); } else { assertNull(configuration.getDepends()); }
    }
    ...
    private void assertContainerInterceptors(String prefix, Element config) {
        ...
        for (int n = 0; n < interceptors.getLength(); n++) {
            ...
            String expected;
            if (tx.length() == 0 || tx.equals("Both")) { expected = prefix + ".Interceptor" + (n + 1); } else if (tx.equals("Container")) {
                expected = prefix + ".Interceptor" + (n + 1) + "c";
            } else { expected = prefix + ".Interceptor" + (n + 1) + "b"; }
    ...
    private void assertContainerPoolConf(String prefix, Element config, Mode mode) {
        ...
        if ((mode != Mode.JBOSS_DTD && hasStandardJBoss == false) || id.length() > 0) { assertEquals(prefix + "-pool-conf", id); }
    ...
    private void assertInvokerProxyBinding(String prefix, int count, InvokerProxyBindingMetaData binding, Mode mode) {
        ...
        if (config.getElementsByTagName("client-interceptors").getLength() > 0) { assertInvokerProxyBindingPFCClientInterceptor(prefix, count, config); }
    }
    ...
    private void assertInvokerProxyBindingPFCClientInterceptor(String prefix, int count, Element config) {
        ...
        for (int n = 0; n < homeInterceptors.getLength(); n++) {
            ...
            String text = interceptor.getTextContent();
            String expected;
            if (callByValue.length() == 0) { expected = "org.jboss.proxy.ejb.HomeInterceptor" + (n + 1) + "." + count; } else {
        ...
        for (int n = 0; n < beanInterceptors.getLength(); n++) {
            ...
            String text = interceptor.getTextContent();
            String expected;
            if (callByValue.length() == 0) { expected = "org.jboss.proxy.ejb.BeanInterceptor" + (n + 1) + "." + count; } else {
    ...
    protected void assertAssemblyDescriptor(JBossMetaData jbossMetaData, Mode mode) {
        ...
        if (mode != Mode.JBOSS_DTD) { assertId("assembly-descriptor", assemblyDescriptorMetaData); }
    ...
    */
    ...
    private void assertExpectedSource(String value, String expectedPrefix, String suffix) {
        if (expectedPrefix == null) { assertNull(value); } else { assertEquals(expectedPrefix + "Configuration" + suffix, value); }
    }
    ...
    protected void assertResourceGroup(String prefix, ResourceInjectionMetaData resourceInjectionMetaData, boolean full, boolean first, Mode mode) {
        if (mode == Mode.JBOSS) {
            ...
            if (first) { assertTrue(resourceInjectionMetaData.isDependencyIgnored()); } else { assertFalse(resourceInjectionMetaData.isDependencyIgnored()); }
        }
