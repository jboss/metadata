...
public class MailSessionsMetaDataMerger {
     ...
     */
    public static MailSessionsMetaData merge(MailSessionsMetaData override, MailSessionsMetaData overriden, String overridenFile,
                                            String overrideFile) {
        ...
        MailSessionsMetaData merged = new MailSessionsMetaData();
        return merge(merged, overriden, override, "mail-session", overridenFile, overrideFile, false);
    }

    /**
     * From JavaEEMetaDataUtil
     */
    private static MailSessionsMetaData merge(MailSessionsMetaData merged, MailSessionsMetaData overriden,
            MailSessionsMetaData mapped, String context, String overridenFile, String overrideFile, boolean mustOverride) {
        if (merged == null)
        ...
        for (MailSessionMetaData original : overriden) {
            String key = original.getKey();
            if (mapped != null && mapped.containsKey(key)) {
                MailSessionMetaData override = mapped.get(key);
                MailSessionMetaData tnew = MailSessionMetaDataMerger.merge(override, original);
                merged.add(tnew);
            } else {
                merged.add(original);
            }
        ...
        if (mapped != null) {
            for (MailSessionMetaData override : mapped) {
    ...
    public static void augment(MailSessionsMetaData dest, MailSessionsMetaData augment,
            MailSessionsMetaData main, boolean resolveConflicts) {
        for (MailSessionMetaData metaData : augment) {
